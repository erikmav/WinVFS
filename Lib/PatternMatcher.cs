...
internal static class PatternMatcher
{
    ...
    public static bool StrictMatchPattern(string expression, string name)
    {
        ...
        if (string.IsNullOrEmpty(name) || expression == null || expression.Length == 0)
        {
            return false;
        }

        // Special case by far the most common wild card search of * or *.*
        if (expression.Equals("*", StringComparison.Ordinal) || expression.Equals("*.*", StringComparison.Ordinal))
        {
            return true;
        }
